...
const getOP = () => {
    ...
    s3.getObject(paramsGO, (err, data) => {
        if (err) {
        console.error('Error retrieving file from S3:', err);
        } else {
        // File content is in data.Body
        const fileContent = data.Body.toString('utf-8');
        console.log('Retrieved file content:', fileContent);
    
        // Further processing of the file content (e.g., parsing JSON)
        // Once you have the file content, you can perform any necessary operations on it
        }
    });
}
